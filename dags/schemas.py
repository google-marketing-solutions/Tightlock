...
def build_schema_type(schema: ProtocolSchema, class_name: str) -> type:
  ...
  default_fields = [("name", str,
                     Field(description=f"Name of the {class_name.lower()}.",
                           immutable=True))]
  default_fields_names = [f[0] for f in default_fields]
  fields = list(default_fields)
  # filter out fields that match default fields
  fields.extend(
      [f for f in schema.fields if f[0].lower() not in default_fields_names])
  schema_type = make_dataclass(schema.class_name,
                               bases=(BaseModel,),
                               fields=fields)
  return schema_type
...
def schema_dag():
  ...
  def retrieve_schemas():
    ...
    for folder_name in (sources_folder, destinations_folder):
      ...
      modules = DagUtils.import_modules_from_folder(folder_name)
