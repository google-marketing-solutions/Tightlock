...
class Destination:
    ...
    def send_data(
        ...
        """
        if len(input_data) == 0:
            print("No rows of user data to send. Skipping.")
            return RunResult(dry_run=dry_run, successful_hits=0, failed_hits=0)

        print(f"Validating {len(input_data)} user records")
        (
            valid_conversions,
            invalid_indices_and_errors,
        ) = self._get_valid_and_invalid_conversions(input_data)
        successfully_uploaded_conversions = []

        print(f"Preparing {len(valid_conversions)} valid rows of {len(input_data)}")
        if not dry_run:
